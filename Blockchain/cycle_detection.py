...
def oneLayerDFS(main_node):
    ...
    for prev_node in layer1_prev:
        ...
        for next_node in main_node.next_list:
            ...
            if timestamp_next - timestamp_prev <= time_limit:
...
def twoLayerDFS(main_node):
    ...
    for layer2_list in layer2_prev:
        for layer2_influx_node in layer2_list:
            ...
            if timestamp_layer1_influx - timestamp_layer2_influx <= time_limit:
            ...
            for layer1_outflux_node in main_node.next_list:
                ...
                if timestamp_layer1_outflux - timestamp_layer1_influx <= time_limit:
                ...
                for layer2_outflux_node in layer1_outflux_node.next_list:
                    ...
                    if timestamp_layer2_outflux - timestamp_layer1_outflux <= time_limit:
...
def summary_file_test():
    sum_file = '/SummaryFile.xlsx'
    df = pd.read_excel(os.getcwd() + sum_file)
    for index, row in df.iterrows():
        name = row['ICO_Name_ib']
        status = str(row['Status(donghwa)'])
        address = row['DistributorAddress']
        print(name, status, address)
        if status != 'OK':
            continue
        distributors = []
