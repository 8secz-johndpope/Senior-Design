...
class CycleClient:
    ...
    def oneLayerDFS(self, main_node):
        cycles = 0
        one_layer_cycles = []
    ...
    def twoLayerDFS(self, main_node):
        cycles = 0
        two_layer_cycles = []
        layer1_prev = main_node.prev_list
        layer2_prev = []
    ...
    def detect_single_layer_cycles(self, name, main_wallets):
        for wallet in main_wallets:
            print('SINGLE LAYER TEST')
            one_layer_cycles, one_layer_list = self.oneLayerDFS(self.oneLayerBFS(wallet))
    ...
    def detect_double_layer_cycles(self, name, main_wallets):
        for wallet in main_wallets:
            print('DOUBLE LAYER TEST')
            two_layer_cycles, two_layer_list = self.twoLayerDFS(self.twoLayerBFS(wallet))
    ...
    def detect_cycles(self, name, main_wallets):
        for wallet in main_wallets:
            print('SINGLE LAYER TEST')
            one_layer_cycles, one_layer_list = self.oneLayerDFS(self.oneLayerBFS(wallet))
            print('DOUBLE LAYER TEST')
    ...
    def test_individual_single_layer(self, name, distributors):
        if not os.path.isdir(os.getcwd() + '/results'):
            os.mkdir(os.getcwd() + '/results')
        print(f'Running individual single layer cycle test on {name} using wallets: {distributors}')
        self.detect_single_layer_cycles(name, distributors)

    def test_individual_double_layer(self, name, distributors):
        if not os.path.isdir(os.getcwd() + '/results'):
            os.mkdir(os.getcwd() + '/results')
        print(f'Running individual double layer cycle test on {name} using wallets: {distributors}')
        self.detect_double_layer_cycles(name, distributors)

    def summary_file_test(self):
        if not os.path.isdir(os.getcwd() + '/results'):
            os.mkdir(os.getcwd() + '/results')
        print('Running full test - Using summary file')
        ...
        for index, row in df.iterrows():
            ...
            if distributors:
                ...
                print(distributors)
                self.detect_cycles(name, distributors)
            else:
                continue

cycleClient = CycleClient()
cycleClient.summary_file_test()
